...
class UserRegisterResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            ### 테이블에 방금 insert한 데이터의 아이디 값을 가져오는 방법
        ...
        return{'result':'success','aCCCESS_TOKEN':access_token},200
...
class UserLoginResource(Resource):
    ...
    def post(self):
        ...
        if len(result_list) == 0:
            return{"errorr":"회원 가입 하세요."}, 400

        # 회원은 맞으니, 비밀번호가 맞는지 체크한다.
        # 로그인한 사람이 막 입력한 비번 :data['password']
        # 회원 가입할때 입력했던 암호화된 비번 :DB에 있다
        # result_list에 들어있고
        ...
        # JWT 토큰을 만들어서, 클라이언트에 응답한다.
        access_token = create_access_token(result_list[0]['id'])
        # access_token을 만들때 유효기간을 가지게 만드는법
        # import datetime
